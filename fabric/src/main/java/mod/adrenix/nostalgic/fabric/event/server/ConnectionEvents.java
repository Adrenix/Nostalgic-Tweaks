    ...
    private static void verify(MinecraftServer server, ServerLoginPacketListenerImpl handler, boolean understood, FriendlyByteBuf buffer, ServerLoginNetworking.LoginSynchronizer synchronizer, PacketSender sender)
    {
        ...
        if (understood)
        {
            ...
            if (!CLIENT_PROTOCOL.equals(SERVER_PROTOCOL))
            {
                ...
                handler.disconnect(ComponentBackport.translatable(MESSAGE, MOD_NAME, ComponentUtil.color(CLIENT_PROTOCOL, 0xF87C73), ComponentUtil.color(SERVER_PROTOCOL, 0xF8BE73), MOD_NAME, SERVER_VERSION, MOD_LINK));
            }
        }
        else
            handler.disconnect(ComponentBackport.translatable("You need %s (%s) to join this server.\n%s", MOD_NAME, SERVER_VERSION, MOD_LINK));
    }
}
