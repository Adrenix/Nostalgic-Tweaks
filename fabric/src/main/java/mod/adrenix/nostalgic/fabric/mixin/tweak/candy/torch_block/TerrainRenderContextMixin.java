...
public abstract class TerrainRenderContextMixin extends AbstractBlockRenderContext
{
     ...
     * changes the target. A better solution that does not involve mixing into the API internals is necessary.
     */
    @WrapOperation(
    ...
    private void nt_fabric_torch_block$wrapEmitBlockQuads(BakedModel model, QuadEmitter quadEmitter, BlockAndTintGetter blockAndTintGetter, BlockState blockState, BlockPos blockPos, Supplier<RandomSource> randomSupplier, Predicate<Direction> cullTest, Operation<Void> operation, @Local(argsOnly = true) PoseStack poseStack)
    {
        if (TorchHelper.isLikeTorch(blockState))
        {
            ...
            TorchHelper.writeVertices(poseStack, blockAndTintGetter, vertexConsumer, model, blockState, blockPos, randomSupplier.get());
        }
        else
            operation.call(model, quadEmitter, blockAndTintGetter, blockState, blockPos, randomSupplier, cullTest);
    }
}
