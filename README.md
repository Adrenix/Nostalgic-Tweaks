...
- [1.20](https://github.com/Adrenix/Nostalgic-Tweaks/tree/1.20)
...
`mc-version+mod-loader+mod-version`. An example version number would be `1.20.2+fabric+2.0.0`.
...
Where `<version>` is replaced by an appropriate version number found on [Modrinth](https://modrinth.com/mod/nostalgic-tweaks/versions).
Ensure you are using a **version number** and not a *version display name*. A version number is formatted as
`mc-version+mod-loader+mod-version`. An example version number would be `1.20.2+forge+2.0.0`.
