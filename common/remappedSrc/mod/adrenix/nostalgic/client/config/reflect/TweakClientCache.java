...
public class TweakClientCache<T>
{
    ...
    {
        if (!NostalgicTweaks.isNetworkVerified() || NetClientUtil.isSingleplayer() || MinecraftClient.getInstance().world == null)
            return true;
    ...
    public Text getSearchGroup()
    {
        ...
        MutableText group = Text.translatable(this.getGroup().getLangKey()).withStyle(Formatting.GOLD);

        if (cat != null || sub != null || emb != null)
            group.append(Formatting.WHITE + "/");

        if (cat != null)
            group.append(Text.translatable(cat.group().getLangKey()).withStyle(Formatting.YELLOW));

        if (sub != null)
        {
            group.append(Text.translatable(sub.group().getCategory().getLangKey()).withStyle(Formatting.YELLOW)).append(Formatting.WHITE + "/");
            group.append(Text.translatable(sub.group().getLangKey()).withStyle(Formatting.GREEN));
        }

        if (emb != null)
        {
            group.append(Text.translatable(emb.group().getSubcategory().getCategory().getLangKey()).withStyle(Formatting.YELLOW)).append(Formatting.WHITE + "/");
            group.append(Text.translatable(emb.group().getSubcategory().getLangKey()).withStyle(Formatting.GREEN)).append(Formatting.WHITE + "/");
            group.append(Text.translatable(emb.group().getLangKey()).withStyle(Formatting.AQUA));
        }
    ...
    public String getTranslation() { return Text.translatable(this.getLangKey()).getString(); }
    public String getTooltipTranslation() { return Text.translatable(this.getTooltipKey()).getString(); }
}
