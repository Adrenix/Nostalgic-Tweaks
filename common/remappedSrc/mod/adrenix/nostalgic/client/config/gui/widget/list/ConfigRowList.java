...
public class ConfigRowList extends AbstractRowList<ConfigRowList.Row>
{
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button)
    {
        ...
        if (this.screen.getConfigTab() == ConfigScreen.ConfigTab.SEARCH && clicked)
            this.screen.getWidgets().getSearchInput().setTextFieldFocused(false);
        return clicked;
    }

    /* Tab Key Support */

    private boolean isInvalidWidget(Row row, ClickableWidget widget)
    {
        ...
        boolean isInactive = (isGroup || isReset || isController) && !widget.isNarratable();
    ...
    public abstract static class AbstractRow<T>
    {
        ...
        protected ConfigRowList.Row create(ClickableWidget controller)
        {
    ...
    // Key Binding Row
    public record BindingRow(KeyBinding mapping)
    {
        public ConfigRowList.Row add()
        {
    ...
    public record SingleCenteredRow(Screen screen, ClickableWidget center)
    {
        public ConfigRowList.Row add()
        {
            List<AbstractWidget> widgets = new ArrayList<>();
    ...
    public record SingleLeftRow(ClickableWidget left, int indent)
    {
        public ConfigRowList.Row add()
        {
            List<AbstractWidget> widgets = new ArrayList<>();
    ...
    // Manual Row Entry
    public record ManualRow(List<ClickableWidget> widgets)
    {
    ...
    public static class CategoryRow
    {
        ...
        public CategoryRow(ConfigRowList list, Text title, Supplier<ArrayList<ConfigRowList.Row>> childrenSupply, Enum<?> id, CatType categoryType)
        {
            this.id = id;
            this.list = list;
            this.title = title;
        ...
        public CategoryRow(ConfigRowList list, Text title, Supplier<ArrayList<ConfigRowList.Row>> childrenSupply, Enum<?> id)
        {
        ...
        private void setGroupMetadata()
        {
            ...
            {
                for (ClickableWidget widget : categories.children)
                {
                    ...
                    if (widget instanceof GroupButton group && this.controller.equals(group))
                    {
                        ...
                        {
                            for (ClickableWidget subWidget : subcategories.children)
                            {
        ...
        private int getHeaderIndex()
        {
            ...
            for (int i = 0; i < this.list.children().size(); i++)
            {
                ...
                for (ClickableWidget widget : this.list.children().get(i).children)
                {
            ...
            for (ConfigRowList.Row cache : this.cache)
            {
                for (ConfigRowList.Row child : this.list.children())
                {
                    if (child.equals(cache))
                    {
                        ...
                        for (ClickableWidget widget : child.children)
                        {
    ...
    public static class Row extends ElementListWidget.Entry<Row>
    {
        ...
        @Nullable public final ClickableWidget controller;
        ...
        public Row(List<ClickableWidget> list, @Nullable ClickableWidget controller, @Nullable TweakClientCache<?> cache)
        {
            this.children = list;
        ...
        public Row(List<ClickableWidget> list, @Nullable TweakClientCache<?> cache) { this(list, null, cache); }
        ...
        private boolean isRowLocked()
        {
            ...
            for (ClickableWidget widget : this.children)
                if (widget instanceof IPermissionWidget && MinecraftClient.getInstance().player != null)
                    return !NetClientUtil.isPlayerOp(MinecraftClient.getInstance().player);

            return false;
        }

        private void renderOnHover(MatrixStack poseStack, Screen screen, int top, int height)
        {
            ...
            int b = rgba[2];
            int a = rgba[3];
            int alpha = MathHelper.clamp(isFaded ? (int) (this.fadeIn * a) : a, 0, 255);

            buffer.begin(VertexFormat.DrawMode.QUADS, VertexFormats.POSITION_COLOR);
            buffer.vertex(matrix, (float) 0, (float) (top + height), z).color(r, g, b, alpha).next();
            buffer.vertex(matrix, (float) screen.width, (float) (top + height), z).color(r, g, b, alpha).next();
            buffer.vertex(matrix, (float) screen.width, (float) (top - 1), z).color(r, g, b, alpha).next();
            buffer.vertex(matrix, (float) 0, (float) (top - 1), z).color(r, g, b, alpha).next();
            tesselator.draw();
        ...
        private void renderTree(MatrixStack poseStack, int top, int height)
        {
        ...
        private static boolean isRowClipped(Screen screen, ClickableWidget widget)
        {
            return widget.x + widget.getWidth() >= screen.width - TEXT_FROM_END;
        }

        @Override
        public List<? extends Selectable> selectableChildren() { return this.children; }
        ...
        public void render(MatrixStack poseStack, int index, int top, int left, int width, int height, int mouseX, int mouseY, boolean isMouseOver, float partialTick)
        {
            ...
            this.fadeIn = MathHelper.clamp(this.fadeIn, 0F, 1F);
            ...
            for (ClickableWidget widget : this.children)
            {
                ...
                if (this.cache != null)
                {
                    Text translation = Text.translatable(tagger == null ? this.cache.getLangKey() : tagger.getTitle());
                    title = this.cache.isSavable() ? translation.shallowCopy().formatted(Formatting.ITALIC) : translation.shallowCopy().formatted(Formatting.RESET);

                    if (isFocused)
                        title = title.shallowCopy().formatted(Formatting.GOLD);
                }
                else if (widget instanceof KeyBindButton)
                {
                    Text translation = Text.translatable(((KeyBindButton) widget).getMapping().getTranslationKey());
                    title = KeyBindButton.isMappingConflicted(((KeyBindButton) widget).getMapping()) ? translation.shallowCopy().formatted(Formatting.RED) : translation.shallowCopy().formatted(Formatting.RESET);
                }
                ...
                Screen.drawTextWithShadow(poseStack, font, title, startX, top + 6 + dy, 0xFFFFFF);
                ...
                if (screen.getConfigTab() == ConfigScreen.ConfigTab.SEARCH)
                {
                    ...
                    cacheY = widget.y + (widget instanceof TextFieldWidget ? -1 : 0);
                }
                ...
                boolean isOverText = (mouseX >= startX && mouseX <= startX + font.getWidth(title)) && (mouseY >= top + 6 && mouseY <= top + 6 + 8);

                if (isEllipsis && isOverText && this.cache != null)
                {
                    screen.renderLast.add(() ->
                        screen.renderTooltip(poseStack, NostalgicUtil.Wrap.tooltip(Text.translatable(this.cache.getTranslation()), 35), mouseX, mouseY))
                    ;
                }
                ...
                if (NostalgicTweaks.isDebugging() && this.isMouseOver(mouseX, mouseY) && this.cache != null)
                {
                    if (screen.getConfigTab() == ConfigScreen.ConfigTab.SEARCH)
                    {
                        ...
                        screen.renderTooltip(poseStack, Text.literal(String.format("Fuzzy Weight: %s%s", color, weight)), mouseX, mouseY);
                    }
                    else
                    {
                        ...
                        lines.add(Text.literal(String.format("Client Cache: %s%s", clientColor, clientCache)));
                        ...
                        if (serverCache != null)
                        {
                            ...
                            lines.add(Text.literal(String.format("Server Cache: %s%s", serverColor, serverCache.getServerCache())));
                        }
