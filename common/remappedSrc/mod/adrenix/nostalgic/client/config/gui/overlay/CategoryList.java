...
public class CategoryList extends Overlay
{
    ...
    private static class TextButton extends ButtonWidget
    {
        /* Widget Constructor Helpers */

        private static int getTextWidth(Text title) { return MinecraftClient.getInstance().textRenderer.getWidth(title); }
        private static int getTextHeight() { return MinecraftClient.getInstance().textRenderer.fontHeight; }
        ...
        public TextButton(ConfigScreen screen, ConfigRowList.Row row, int color, int startX, int startY, Text title, ButtonWidget.PressAction onClick)
        {
        ...
        public void render(MatrixStack poseStack, int mouseX, int mouseY, float partialTick)
        {
            ...
            drawTextWithShadow(poseStack, this.screen.getFont(), isSelected ? this.title.shallowCopy().formatted(Formatting.GOLD) : this.title, this.x, this.y, highlight);
        }
    ...
    private static class TextRowList extends AbstractRowList<TextRow>
    {
        ...
        public record EntryRow(TextRowList list, ConfigRowList.Row row, @Nullable GroupButton group, Text title, int indent, ButtonWidget.PressAction onClick)
        {
            public TextRow add()
            {
    ...
    private static class TextRow extends ElementListWidget.Entry<TextRow>
    {
        public final List<ClickableWidget> children;

        public TextRow(List<ClickableWidget> children)
        {
    ...
    public void generateWidgets()
    {
        ...
        this.list = new TextRowList(screen, width, height, startY, endY, screen.getFont().fontHeight + 2);
        ...
        for (ConfigRowList.Row row : this.all.children())
        {
            ...
            ButtonWidget.PressAction jump = button -> {
            ...
            if (row.cache != null)
                this.list.addRow(new TextRowList.EntryRow(this.list, row, null, Text.translatable(row.cache.getLangKey()), indent, jump).add());
        }
    }

    private boolean isInvalidWidget(TextRow row, ClickableWidget widget)
    {
    ...
    public void onRender(MatrixStack poseStack, int mouseX, int mouseY, float partialTick)
    {
        ...
        // Draw top left
        drawTexture(poseStack, startX, startY, U_TOP_LEFT_CORNER, V_TOP_LEFT_CORNER, W_TOP_LEFT_CORNER, H_TOP_LEFT_CORNER);

        // Draw top width
        for (int i = 0; i < drawWidth; i++)
            drawTexture(poseStack, i + startX + W_TOP_LEFT_CORNER, startY, U_TOP_BAR, V_TOP_BAR, 1, H_TOP_BAR);

        // Draw top right
        drawTexture(poseStack, startX + W_TOP_LEFT_CORNER + drawWidth, startY, U_TOP_RIGHT_CORNER, V_TOP_RIGHT_CORNER, W_TOP_RIGHT_CORNER, H_TOP_RIGHT_CORNER);

        // Draw sidebars
        for (int i = 0; i < drawHeight; i++)
        {
            drawTexture(poseStack, startX, i + startY + H_TOP_LEFT_CORNER, U_LEFT_BAR, V_LEFT_BAR, W_LEFT_BAR, 1);
            drawTexture(poseStack, startX + W_TOP_LEFT_CORNER + drawWidth + 8, i + startY + H_TOP_RIGHT_CORNER, U_RIGHT_BAR, V_RIGHT_BAR, W_RIGHT_BAR, 1);
        }

        // Draw bottom left
        drawTexture(poseStack, startX, startY + H_TOP_LEFT_CORNER + drawHeight, U_BOTTOM_LEFT_CORNER, V_BOTTOM_LEFT_CORNER, W_BOTTOM_LEFT_CORNER, H_BOTTOM_LEFT_CORNER);

        // Draw bottom width
        for (int i = 0; i < drawWidth; i++)
            drawTexture(poseStack, i + startX + W_BOTTOM_LEFT_CORNER, startY + H_TOP_LEFT_CORNER + drawHeight, U_BOTTOM_BAR, V_BOTTOM_BAR, 1, H_BOTTOM_BAR);

        // Draw bottom right
        drawTexture(poseStack, startX + W_BOTTOM_LEFT_CORNER + drawWidth, startY + H_TOP_RIGHT_CORNER + drawHeight, U_BOTTOM_RIGHT_CORNER, V_BOTTOM_RIGHT_CORNER, W_BOTTOM_RIGHT_CORNER, H_BOTTOM_RIGHT_CORNER);
        ...
        drawTexture(poseStack, hintX, closeY, isOverHint ? U_HINT_ON : U_HINT_OFF, isOverHint ? V_HINT_ON : V_HINT_OFF, HINT_SQUARE, HINT_SQUARE);
        ...
        drawString(Text.translatable(NostalgicLang.Gui.GUI_OVERLAY_LIST), startX + 20, startY + 4, color);
        ...
        {
            List<Text> tooltip = NostalgicUtil.Wrap.tooltip(Text.translatable(NostalgicLang.Gui.GUI_OVERLAY_LIST_HINT), 36);
            screen.renderTooltip(poseStack, tooltip, mouseX, mouseY);
        }
