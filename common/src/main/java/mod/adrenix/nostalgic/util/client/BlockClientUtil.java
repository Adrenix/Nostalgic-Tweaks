...
public abstract class BlockClientUtil
{
     ...
     * @param random Used to get a random integer from.
     ...
     */
    public static BlockPos getRandomPos(Random random, int bound)
    {
        return new BlockPos(random.nextInt(bound), random.nextInt(bound), random.nextInt(bound));
    }
     ...
     */
    public static void oldTorch(PoseStack poseStack, VertexConsumer consumer, BakedModel model, BlockState state, BlockPos position, Random random)
    {
