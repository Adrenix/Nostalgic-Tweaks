...
public abstract class ModAsset
{
    ...
    /**
     * Get a new resource location from the mod's gui sprites directory. Do <b color=red>not</b> include {@code .png} in
     * the path. This method will append it.
     *
     * @param path A path in the mod's {@code sprites} directory.
     * @return A new {@link ResourceLocation} instance where the returned-path references a texture location, and
     * appears as {@code nostalgic_tweaks:textures/gui/sprites/$path} where {@code $path} is the given {@code path}
     * argument.
     */
    public static ResourceLocation sprite(String path)
    {
        return get("textures/gui/sprites/" + path + ".png");
    }
