...
public abstract class OverworldFogRenderer
{
    ...
    private static void calculateLightInfluence(final float[] FOG_COLOR)
    {
        ...
        int maxLight = ClientWorldHelper.getMaxLight(camera.getEntity());

        if (!VoidFogRenderer.isGameOverride(camera))
            maxLight = ClientWorldHelper.getSkyLight(camera.getEntity());
    ...
    {
        final float[] ENV_RGB = ClientWorldHelper.getStandardEnvironmentInfluence();
    ...
    private static float[] getColorFromBiome()
    {
        ...
        float temp = ClientWorldHelper.getBiomeTemperature() / 2.0F;
        int renderDistance = GameUtil.getRenderDistance();
