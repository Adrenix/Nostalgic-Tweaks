...
public class ColorPickerOverlay extends Overlay
{
    ...
    public void onRender(PoseStack poseStack, int mouseX, int mouseY, float partialTick)
    {
        ...
        drawString(ComponentBackport.translatable(LangUtil.Gui.OVERLAY_COLOR), startX + 19, startY + 5, color);
        ...
        if (isOverIcon)
        {
            List<Component> tooltip = TextUtil.Wrap.tooltip(ComponentBackport.translatable(LangUtil.Gui.OVERLAY_DRAG_TIP), 36);
            screen.renderComponentTooltip(poseStack, tooltip, mouseX, mouseY);
        }

        if (isOverHint && this.hint)
        {
            List<Component> tooltip = TextUtil.Wrap.tooltip(ComponentBackport.translatable(LangUtil.Gui.OVERLAY_COLOR_HINT), 36);
            screen.renderComponentTooltip(poseStack, tooltip, mouseX, mouseY);
        }
