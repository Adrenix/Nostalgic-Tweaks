...
public class StateButton extends OverlapButton
{
    ...
    private static Component getText(StateWidget widget)
    {
        return switch (widget)
        {
            ...
            default -> ComponentBackport.empty();
        };
    }
    ...
    private List<Component> getTooltip()
    {
        ...
        Component hide = ComponentBackport.translatable(LangUtil.Gui.STATE_HIDE).withStyle(ChatFormatting.GRAY);
        Component shift = ComponentBackport.translatable(LangUtil.Gui.STATE_SHIFT).withStyle(ChatFormatting.GRAY);

        Component title = switch (this.widget)
        {
            case TAG -> ComponentBackport.translatable(LangUtil.Gui.STATE_TAG).withStyle(ChatFormatting.GREEN);
            case NUKE -> ComponentBackport.translatable(LangUtil.Gui.STATE_NUKE).withStyle(ChatFormatting.RED);
            case SWING -> ComponentBackport.translatable(LangUtil.Gui.STATE_SWING).withStyle(ChatFormatting.BLUE);
            case CLEAR -> ComponentBackport.translatable(LangUtil.Gui.STATE_CLEAR).withStyle(ChatFormatting.RED);
            case FUZZY -> ComponentBackport.translatable(LangUtil.Gui.STATE_FUZZY).withStyle(ChatFormatting.GOLD);
            case BUBBLE -> ComponentBackport.translatable(LangUtil.Gui.STATE_BUBBLE).withStyle(ChatFormatting.AQUA);
            case FILTER -> ComponentBackport.translatable(LangUtil.Gui.STATE_FILTER).withStyle(ChatFormatting.GOLD);
            case LIGHTNING -> ComponentBackport.translatable(LangUtil.Gui.STATE_LIGHTNING).withStyle(ChatFormatting.YELLOW);
        };

        List<Component> wrap = switch (this.widget)
        {
            case TAG -> TextUtil.Wrap.tooltip(ComponentBackport.translatable(LangUtil.Gui.STATE_TAG_TOOLTIP), 35);
            case NUKE -> TextUtil.Wrap.tooltip(ComponentBackport.translatable(LangUtil.Gui.STATE_NUKE_TOOLTIP), 35);
            case SWING -> TextUtil.Wrap.tooltip(ComponentBackport.translatable(LangUtil.Gui.STATE_SWING_TOOLTIP), 30);
            case CLEAR -> TextUtil.Wrap.tooltip(ComponentBackport.translatable(LangUtil.Gui.STATE_CLEAR_TOOLTIP), 40);
            case FUZZY -> TextUtil.Wrap.tooltip(ComponentBackport.translatable(LangUtil.Gui.STATE_FUZZY_TOOLTIP), 35);
            case BUBBLE -> TextUtil.Wrap.tooltip(ComponentBackport.translatable(LangUtil.Gui.STATE_BUBBLE_TOOLTIP), 35);
            case FILTER -> TextUtil.Wrap.tooltip(ComponentBackport.translatable(LangUtil.Gui.STATE_FILTER_TOOLTIP), 40);
            case LIGHTNING -> TextUtil.Wrap.tooltip(ComponentBackport.translatable(LangUtil.Gui.STATE_LIGHTNING_TOOLTIP), 35);
        };
