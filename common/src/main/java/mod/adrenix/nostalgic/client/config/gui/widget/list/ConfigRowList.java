    ...
    public static class Row extends ContainerObjectSelectionList.Entry<Row>
    {
        ...
        public void render(PoseStack poseStack, int index, int top, int left, int width, int height, int mouseX, int mouseY, boolean isMouseOver, float partialTick)
        {
            ...
            for (AbstractWidget widget : this.children)
            {
                ...
                Component title = ComponentBackport.empty();

                if (this.tweak != null)
                {
                    Component translation = ComponentBackport.translatable(tagger == null ? this.tweak.getLangKey() : tagger.getTitle());
                ...
                {
                    Component translation = ComponentBackport.translatable(((KeyBindButton) widget).getMapping().getName());
                ...
                if (NostalgicTweaks.isDebugging() && this.isMouseOver(mouseX, mouseY) && this.tweak != null)
                {
                    if (screen.getConfigTab() == ConfigScreen.ConfigTab.SEARCH)
                    {
                        ...
                        screen.renderTooltip(poseStack, ComponentBackport.literal(String.format("Fuzzy Weight: %s%s", color, weight)), mouseX, mouseY);
                    }
                    else
                    {
