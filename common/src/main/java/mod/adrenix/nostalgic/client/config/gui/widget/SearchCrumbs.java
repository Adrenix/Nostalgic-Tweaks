...
public class SearchCrumbs extends AbstractWidget
{
    ...
    private record Crumb(int startX, TweakClientCache<?> tweak, MutableComponent text, JumpType jump)
    {
        public int getEndX()
        {
            ...
            return this.startX + font.width(text) + font.width(ComponentBackport.literal("/"));
        }
    }
    ...
    private final MutableComponent slashText = ComponentBackport.literal("/").withStyle(ChatFormatting.WHITE);
    ...
    {
        super(ConfigRowList.TEXT_START, 0, 0, 0, ComponentBackport.empty());

        this.startX = ConfigRowList.TEXT_START;

        String groupKey = tweak.getGroup().getLangKey();
        Crumb groupCrumb = new Crumb(startX, tweak, ComponentBackport.translatable(groupKey).withStyle(ChatFormatting.GOLD), JumpType.GROUP);
        ...
        if (tweak.getCategory() != null)
        {
            ...
            MutableComponent catText = ComponentBackport.translatable(catKey).withStyle(ChatFormatting.YELLOW);
        ...
        if (tweak.getSubcategory() != null)
        {
            ...
            MutableComponent catText = ComponentBackport.translatable(catKey).withStyle(ChatFormatting.YELLOW);
            MutableComponent subText = ComponentBackport.translatable(subKey).withStyle(ChatFormatting.GREEN);
        ...
        if (tweak.getEmbed() != null)
        {
            ...
            MutableComponent catText = ComponentBackport.translatable(catKey).withStyle(ChatFormatting.YELLOW);
            MutableComponent subText = ComponentBackport.translatable(subKey).withStyle(ChatFormatting.GREEN);
            MutableComponent embText = ComponentBackport.translatable(embKey).withStyle(ChatFormatting.AQUA);
    ...
    {
        Font font = Minecraft.getInstance().font;
        int startX = crumb.getEndX() - font.width(ComponentBackport.literal("/"));
