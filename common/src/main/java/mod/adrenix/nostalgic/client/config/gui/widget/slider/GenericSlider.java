...
public class GenericSlider extends AbstractSliderButton
{
    ...
    {
        super(x, y, width, height, ComponentBackport.empty(), (double) tweak.getValue());
    ...
    {
        super(x, y, width, height, ComponentBackport.empty(), (double) current.get());
    ...
    public void updateMessage()
    {
        ...
        {
            this.setMessage(ComponentBackport.empty());
            return;
        }
        ...
        if (this.sliderType == TweakGui.SliderType.SWING)
            title = ComponentBackport.translatable(LangUtil.Gui.SETTINGS_SPEED).getString();
        else if (this.sliderType == TweakGui.SliderType.INTENSITY)
        {
            suffix = "%";
            title = ComponentBackport.translatable(LangUtil.Gui.SETTINGS_INTENSITY).getString();
        }
        else if (this.sliderType == TweakGui.SliderType.CLOUD)
        {
            title = switch (this.current.get())
            {
                case 108 -> ComponentBackport.translatable(LangUtil.Gui.SETTINGS_ALPHA).getString();
                case 128 -> ComponentBackport.translatable(LangUtil.Gui.SETTINGS_BETA).getString();
                case 192 -> ComponentBackport.translatable(LangUtil.Gui.SETTINGS_MODERN).getString();
                default -> ComponentBackport.translatable(LangUtil.Gui.SETTINGS_CUSTOM).getString();
            };
        }

        if (this.sliderData != null && !this.sliderData.langKey().isEmpty())
            title = ComponentBackport.translatable(this.sliderData.langKey()).getString();
        ...
        this.setMessage(ComponentBackport.literal(text));
    }
