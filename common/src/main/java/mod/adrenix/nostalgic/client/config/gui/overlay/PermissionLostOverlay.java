...
public class PermissionLostOverlay extends ListScreenOverlay<PermissionLostOverlay.WidgetProvider>
{
    ...
    public PermissionLostOverlay()
    {
        super
        (
            ComponentBackport.translatable(LangUtil.Gui.OVERLAY_PERMISSION),
            OVERLAY_WIDTH,
            OVERLAY_HEIGHT,
            OverlayFlag.LOCKED
        );
    ...
    protected class WidgetProvider extends AbstractWidgetProvider
    {
        ...
        {
            String header = ComponentBackport.translatable(LangUtil.Gui.OVERLAY_PERMISSION_HEADER).getString();

            return new TextWidget
            (
                ComponentBackport.literal(ChatFormatting.DARK_RED + header),
                TextAlign.CENTER,
                this.getStartX() + 2,
                this.getStartY(),
        ...
        private TextWidget createMessageText()
        {
            return new TextWidget
            (
                ComponentBackport.translatable(LangUtil.Gui.OVERLAY_PERMISSION_MESSAGE),
                TextAlign.LEFT,
                this.getStartX(),
        ...
        private Button createWithoutSaving()
        {
            ...
            (
                ...
                ComponentBackport.translatable(LangUtil.Gui.BUTTON_EXIT_NO_SAVE),
                (button) ->
                {
                    Overlay.close();
        ...
        private Button createSaveLocally()
        {
            ...
            (
                ...
                this.getButtonWidth(),
                SettingsScreen.BUTTON_HEIGHT,
                ComponentBackport.translatable(LangUtil.Gui.BUTTON_EXIT_LOCAL_SAVE),
                (button) ->
                {
                    Overlay.close();
