...
public class StringSetOverlay implements DeletableSetOverlay<String, StringSet>
{
    ...
    public void onAdd()
    {
        if (this.getListing().genericType().equals(String.class))
            new StringOverlay(this.set::getSuggestion, this.set::containsKey, this::createListRows, this::addString).open();
        else
    ...
    public @Nullable AbstractRow<?, ?> getRow(String key, String value)
    {
        ...
        ButtonWidget edit = ButtonWidget.create(Lang.Button.EDIT)
            .icon(Icons.PENCIL)
            .onPress(() -> new StringOverlay(this.set::getSuggestion, this.set::containsKey, onEmptyEdit, onEditFinish, value).open())
