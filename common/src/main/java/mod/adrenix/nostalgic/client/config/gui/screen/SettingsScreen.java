...
public class SettingsScreen extends Screen
{
    ...
    {
        this(parent, ComponentBackport.translatable(LangUtil.Gui.SETTINGS_TITLE), isRedirected);
    }
    ...
    protected void init()
    {
        ...
        this.addButton
        (
            ComponentBackport.translatable(LangUtil.Config.CONFIG_TITLE), (button) ->
                this.minecraft.setScreen(new ConfigScreen(this))
        );

        // Config Presets (WIP)
        Button preset = new Button(0, 0, SMALL_WIDTH, BUTTON_HEIGHT, ComponentBackport.translatable(LangUtil.Gui.SETTINGS_PRESETS), (button) -> {});
        preset.active = false;

        this.buttons.add(preset);

        // Support
        this.addButton(ComponentBackport.translatable(LangUtil.Gui.SETTINGS_SUPPORT), LinkUtil.onPress(LinkLocation.KO_FI));

        // Discord
        this.addButton(ComponentBackport.translatable(LangUtil.Gui.SETTINGS_DISCORD), LinkUtil.onPress(LinkLocation.DISCORD));

        // Golden Days Button
        this.addButton(ComponentBackport.translatable(LangUtil.Gui.SETTINGS_GOLDEN_DAYS), LinkUtil.onPress(LinkLocation.GOLDEN_DAYS));

        // Done Button
        this.addButton(ComponentBackport.translatable(LangUtil.Vanilla.GUI_DONE), (button) -> this.onClose());
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTick)
    {
        ...
        else
        {
            ...
            Component hint = ComponentBackport.literal("Debug (Ctrl + Shift + D)").withStyle(ChatFormatting.DARK_GRAY);
    ...
    private void renderSupportToggle(PoseStack poseStack, int mouseX, int mouseY)
    {
        MutableComponent button = DonatorBanner.isOpen() ?
            ComponentBackport.literal("\u274c").withStyle(ChatFormatting.BOLD).withStyle(ChatFormatting.RED) :
            ComponentBackport.literal("\u2764").withStyle(ChatFormatting.RED)
        ;
