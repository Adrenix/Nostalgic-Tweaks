...
public class NostalgicProgressScreen extends Screen implements ProgressListener
{
    ...
    private void setHeaderAndStage()
    {
        ...
        if (header != null && header.getString().equals(ComponentBackport.translatable("menu.savingLevel").getString()))
        {
            this.setHeader(null);
            this.setStage(ComponentBackport.translatable(LangUtil.Gui.LEVEL_SAVING));
        ...
        if (isTextNeeded && (!isMultiplayer || isConnectedLevel) && isChangingLevel)
        {
            ...
            {
                this.setHeader(ComponentBackport.translatable(LangUtil.Gui.LEVEL_ENTER_NETHER));
                this.setStage(ComponentBackport.translatable(LangUtil.Gui.LEVEL_BUILDING));
            }
            else if (currentLevel == Level.END)
            {
                this.setHeader(ComponentBackport.translatable(LangUtil.Gui.LEVEL_ENTER_END));
                this.setStage(ComponentBackport.translatable(LangUtil.Gui.LEVEL_BUILDING));
            }
            else if (currentLevel == Level.OVERWORLD)
            {
                ...
                {
                    this.setHeader(ComponentBackport.translatable(LangUtil.Gui.LEVEL_LEAVING_NETHER));
                    this.setStage(ComponentBackport.translatable(LangUtil.Gui.LEVEL_BUILDING));
                }
                else if (previousLevel == Level.END)
                {
                    this.setHeader(ComponentBackport.translatable(LangUtil.Gui.LEVEL_LEAVING_END));
                    this.setStage(ComponentBackport.translatable(LangUtil.Gui.LEVEL_BUILDING));
                }
            }
            ...
            if (this.stage == null)
            {
                this.setHeader(ComponentBackport.translatable(LangUtil.Gui.LEVEL_LOADING));
                this.setStage(ComponentBackport.translatable(LangUtil.Gui.LEVEL_BUILDING));
            }
        }
    }
