...
public abstract class CustomSwings
{
    ...
    public static String getLocalizedItem(Map.Entry<String, Integer> entry)
    {
        ...
        if (getItemKey(item).equals("minecraft:air"))
            if (isValidEntry(item, entry))
                return new TranslatableComponent(NostalgicLang.Gui.CUSTOMIZE_HAND).getString();
            else
                return new TranslatableComponent(NostalgicLang.Gui.CUSTOMIZE_UNKNOWN).getString();
    ...
    public static List<Component> rangeTooltip()
    {
        ...
        Component alpha = new TranslatableComponent(NostalgicLang.Gui.CUSTOMIZE_RANGE_TOOLTIP_0);
        Component modern = new TranslatableComponent(NostalgicLang.Gui.CUSTOMIZE_RANGE_TOOLTIP_1);
        Component photo = new TranslatableComponent(NostalgicLang.Gui.CUSTOMIZE_RANGE_TOOLTIP_2);
    ...
    public static Component removeTooltip(Map.Entry<String, Integer> entry)
    {
        ...
        return new TranslatableComponent(NostalgicLang.Gui.CUSTOMIZE_REMOVE_TOOLTIP, item).withStyle(ChatFormatting.RED);
    }

    public static Component undoTooltip(Map.Entry<String, Integer> entry)
    {
        Component item = new TextComponent(getLocalizedItem(entry)).withStyle(ChatFormatting.WHITE);
        return new TranslatableComponent(NostalgicLang.Gui.CUSTOMIZE_UNDO_TOOLTIP, item).withStyle(ChatFormatting.GREEN);
    }
}
