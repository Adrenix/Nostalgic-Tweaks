...
public class TextTitle<V> extends AbstractWidget
{
    ...
    {
        super(START_X, START_Y, WIDTH, HEIGHT, ComponentBackport.empty());
    ...
    private String getDefaultTitle(ListMapScreen<?> mapScreen)
    {
        ...
        String prefix = ComponentBackport.translatable(LangUtil.Gui.SLIDER_VALUE).getString();
        String color = ChatFormatting.YELLOW.toString();
        String gray = ChatFormatting.GRAY.toString();

        if (listId == ListId.CUSTOM_FOOD_STACKING)
            prefix = ComponentBackport.translatable(LangUtil.Gui.SLIDER_STACK).getString();

        if (listId == ListId.CUSTOM_FOOD_HEALTH)
        {
            prefix = ComponentBackport.translatable(LangUtil.Gui.SLIDER_HEARTS).getString();
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTick)
    {
        ...
        Component entryTitle = ComponentBackport.literal(itemName);
        Component literalKey = ComponentBackport.translatable(LangUtil.Gui.LIST_ITEM_KEY, this.resourceKey);
        ...
        if (this.removeType == RemoveType.DEFAULT && listScreen instanceof ListMapScreen<?> mapScreen)
            entryTitle = ComponentBackport.literal(entryTitle.getString() + this.getDefaultTitle(mapScreen));
        ...
        if (isItalics)
            entryTitle = ComponentBackport.literal(ChatFormatting.ITALIC + entryTitle.copy().getString());
        ...
        if (isTitleRed || isOverridden)
            entryTitle = ComponentBackport.literal(ChatFormatting.RED + entryTitle.copy().getString());

        if (isInvalid)
            entryTitle = ComponentBackport.literal(ChatFormatting.GOLD + entryTitle.copy().getString());
        else if (listScreen.isItemAdded(itemStack) && this.removeType != RemoveType.DEFAULT)
            entryTitle = ComponentBackport.literal(ChatFormatting.GREEN + entryTitle.copy().getString());
        ...
        {
            String saved = ComponentBackport.translatable(LangUtil.Gui.LIST_SAVED_ITEMS).getString();
            entryTitle = ComponentBackport.literal(entryTitle.getString() + ChatFormatting.RED + " (" + saved + ")");
        }
