...
public abstract class MixinConfig
{
    ...
    {
        if (tweak != null && !tweak.isLoaded())
        {
            tweak.setEnabled();

            // Server side status syncing
            if (NostalgicTweaks.isServer())
            {
                TweakServerCache<?> cache = TweakServerCache.get(tweak.getGroup(), tweak.getKey());
                if (cache != null)
                {
                    ...
                    if (NostalgicTweaks.getServer() != null)
    ...
    private static <T> T getSidedTweak(T client, T server) { return NostalgicTweaks.isServer() ? server : client; }
    ...
    public static class Candy
    {
        ...
        public static boolean oldChestVoxel() { return isModEnabled(CandyTweak.CHEST_VOXEL) && getSidedTweak(CANDY.oldChestVoxel, SERVER_CANDY.oldChestVoxel); }
        ...
        public static boolean oldItemMerging() { return isModEnabled(CandyTweak.ITEM_MERGING) && getSidedTweak(CANDY.oldItemMerging, SERVER_CANDY.oldItemMerging); }
        ...
        public static boolean oldWaterLighting() { return isModEnabled(CandyTweak.WATER_LIGHTING) && getSidedTweak(CANDY.oldWaterLighting, SERVER_CANDY.oldWaterLighting); }
        ...
        public static boolean oldSquareBorder() { return isModEnabled(CandyTweak.SQUARE_BORDER) && getSidedTweak(CANDY.oldSquareBorder, SERVER_CANDY.oldSquareBorder); }
        ...
        public static TweakVersion.Hotbar getHotbar() { return getVersion(CandyTweak.CREATIVE_HOTBAR, getSidedTweak(CANDY.oldCreativeHotbar, SERVER_CANDY.oldCreativeHotbar)); }
